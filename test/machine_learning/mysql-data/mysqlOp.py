...
select max(ID) from {0} group by {1}
    ...
    def wild(self, d):
        s = self.getStr(d,'LIKE')
        sql = "SELECT * FROM {0} WHERE {1}".format(self.table, s)
        self.mycursor.execute(sql)
        return self.mycursor.fetchall()

    def getStr(self, d,relation):
        return reduce(lambda acc, key: "{0} {2} '%{1}%'".format(key, d[key],relation), d, "")
    ...
    def delete(self):
        sql = "DELETE FROM {0} WHERE {1}".format(self.table)
        self.mycursor.execute(sql)
        self.mydb.commit()
